...
const StudentDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans print:bg-white overflow-x-hidden">
      {/* --- HEADER --- */}
      <header className="bg-white border-b px-4 md:px-6 py-4 flex justify-between items-center print:hidden sticky top-0 z-20 shadow-sm w-full">
        <div className="flex items-center gap-3 w-full">
            <Avatar><AvatarImage src={student.passport_url} /><AvatarFallback>ST</AvatarFallback></Avatar>
            <div className="flex-1 min-w-0">
                <h1 className="font-bold text-sm md:text-lg truncate">{student.full_name}</h1>
                <p className="text-xs text-slate-500 truncate">{student.admission_number}</p>
            </div>
        </div>
        <Button variant="destructive" size="sm" onClick={() => navigate('/login')} className="ml-2 shrink-0">
      ...
      <main className="p-4 md:p-8 container mx-auto max-w-5xl">
        ...
        <Tabs defaultValue="results" value={activeTab} onValueChange={setActiveTab} className="space-y-6 print:hidden w-full">
            ...
            <TabsContent value="results">
                {/* CONTROLS */}
                <div className="mb-8 flex flex-col md:flex-row gap-4 justify-between items-start md:items-end print:hidden w-full">
                    ...
                    {results.length > 0 && <Button onClick={handleDownloadPDF} disabled={downloading} className="bg-blue-600 hover:bg-blue-700 text-white w-full md:w-auto"><Download className="w-4 h-4 mr-2"/> {downloading ? "Generating PDF..." : "Download Result"}</Button>}
                </div>

                {/* THE RESULT SHEET CARD (Wrapped in overflow-x-auto for mobile swiping) */}
                <div className="w-full overflow-x-auto pb-6">
                    <div className="min-w-[800px] flex justify-center">
                        <div id="report-card" className="bg-white w-[210mm] min-h-[297mm] p-8 shadow-2xl text-slate-900 border border-gray-100 shrink-0">
                            {/* SCHOOL HEADER */}
                    ...
                    <CardContent className="space-y-8">
                        <div className="flex flex-col md:flex-row gap-8 items-center justify-center">
                            ...
                            <div className="relative group shrink-0">
                                <Avatar className="w-32 h-32 md:w-40 md:h-40 border-4 border-slate-100 shadow-md"><AvatarImage src={student.passport_url} className="object-cover" /><AvatarFallback className="text-4xl">ST</AvatarFallback></Avatar>
                        ...
                        {/* Bio Data Grid */}
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 bg-slate-50 p-4 md:p-6 rounded-lg border">
                            <div className="space-y-1"><Label className="text-[10px] md:text-xs text-gray-500 uppercase">Full Name</Label><div className="font-bold text-base md:text-lg break-words">{student.full_name}</div></div>
                            <div className="space-y-1"><Label className="text-[10px] md:text-xs text-gray-500 uppercase">Admission Number</Label><div className="font-mono font-bold text-blue-600 break-words">{student.admission_number}</div></div>
                            <div className="space-y-1"><Label className="text-[10px] md:text-xs text-gray-500 uppercase">Current Class</Label><div className="font-medium break-words">{student.class?.name || "Unassigned"}</div></div>
                            <div className="space-y-1"><Label className="text-[10px] md:text-xs text-gray-500 uppercase">Gender</Label><div className="font-medium">{student.gender}</div></div>
                            <div className="space-y-1"><Label className="text-[10px] md:text-xs text-gray-500 uppercase">Date of Birth</Label><div className="font-medium flex items-center gap-2"><Calendar className="w-4 h-4 text-gray-400 shrink-0"/> <span className="truncate">{student.date_of_birth || "Not set"}</span></div></div>
                            <div className="space-y-1"><Label className="text-[10px] md:text-xs text-gray-500 uppercase">Age</Label><div className="font-medium">{calculateAge(student.date_of_birth)} Years Old</div></div>
                            <div className="space-y-1"><Label className="text-[10px] md:text-xs text-gray-500 uppercase">Parent Contact</Label><div className="font-medium flex items-center gap-2"><Phone className="w-4 h-4 text-gray-400 shrink-0"/> <span className="truncate">{student.emergency_contact}</span></div></div>
                            <div className="space-y-1"><Label className="text-[10px] md:text-xs text-gray-500 uppercase">Login PIN</Label><div className="font-mono font-bold text-gray-600 tracking-widest flex items-center gap-2"><Hash className="w-4 h-4 text-gray-400 shrink-0"/> {student.pin_code}</div></div>
                        </div>
                    </CardContent>
                </Card>
            </TabsContent>
