...
const HeadTeacherDashboard = () => {
  ...
  const [primaryStats, setPrimaryStats] = useState({ students: 0, staff: 0 });
  ...
  const loadData = async () => {
    ...
    if (sf) {
        const priStaff = sf.filter(s => s.assigned_section === 'primary' || s.assigned_section === 'nursery' || s.assigned_section === 'all' || s.role === 'head_teacher' || s.role === 'proprietor');
        setStaff(priStaff); setPrimaryStats(prev => ({...prev, staff: priStaff.length}));
    }
  };
  ...
  const handleUpdateSettings = async () => {
    ...
    setLoading(false);
    if(!error) { toast({ title: "Updated", description: "School Session & Term Updated!"}); loadData(); }
  };

  const handleRegisterStudent = async () => {
    setLoading(true);
    const selectedClass = primaryClasses.find(c => c.id === newStudent.classId);
    if (!selectedClass) { toast({variant: "destructive", title: "Select Class"}); setLoading(false); return;}
    const rand = Math.floor(1000 + Math.random() * 9000);
    const autoAdmissionNo = selectedClass.section === 'nursery' ? `AMS/NUR/${rand}` : `AMS/PRI/${rand}`;
 ...
 const fetchBatches = async (primaryClassIds: string[]) => {
  ...
  const downloadBroadsheet = () => {
      ...
      const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.setAttribute("download", `Primary_Broadsheet_${cls?.name}_${schoolSettings.current_term}.csv`); document.body.appendChild(link); link.click(); document.body.removeChild(link);
  };

  const toggleStudentStatus = async (id: string, currentStatus: boolean) => { await supabase.from('students').update({ is_active: !currentStatus }).eq('id', id); setPupils(prev => prev.map(s => s.id === id ? { ...s, is_active: !currentStatus } : s)); };
      ...
      <main className="p-4 md:p-6 container mx-auto max-w-6xl">
        <Tabs defaultValue="overview" value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            <TabsList className="flex flex-wrap h-auto p-1 bg-slate-200/50 justify-start print-hide">
                ...
                <TabsTrigger value="pupils"><Baby className="w-4 h-4 mr-2"/> Pupils</TabsTrigger>
            ...
            <Dialog open={!!selectedBatch} onOpenChange={(o) => !o && setSelectedBatch(null)}>
                <DialogContent className="max-w-4xl p-0 overflow-hidden bg-white rounded-xl border-none print-hide">
                    ...
                    <div className="p-6 max-h-[50vh] overflow-y-auto">
                        ...
                        <div className="mt-8 text-right border-t pt-4"><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Uploaded By</p><p className="font-bold text-green-900 text-lg font-serif">{selectedBatch?.uploaded_by}</p></div>
                    </div>
                    <div className="p-6 bg-slate-50 border-t flex justify-end gap-4">
                        {!selectedBatch?.records[0]?.is_approved ? ( <><Button onClick={() => handleDeleteBatch(selectedBatch, false)} variant="outline" className="text-red-600">Reject Batch</Button><Button onClick={() => handleApproveBatch(selectedBatch)} className="bg-green-900 text-white">Approve Batch</Button></> ) : ( <Button onClick={() => handleDeleteBatch(selectedBatch, true)} variant="destructive"><Trash2 className="w-4 h-4 mr-2"/> Delete Approved Batch</Button> )}
                    </div>
                </DialogContent>
            </Dialog>
